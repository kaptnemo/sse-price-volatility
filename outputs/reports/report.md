...
- **Training period**: 2000-01-01 – 2024-06-30 (5930 trading days)
- **Test period**: 2024-07-01 – 2024-12-31 (124 trading days)
...
*No ARIMA results provided. Run `arima_model.residual_diagnostics()` and pass the output to `generate_report()` to embed actual statistics.*

Diagnostics to check:
- Ljung-Box test on residuals: p-values should be > 0.05 (no serial correlation).
- Ljung-Box test on squared residuals: significant p-values indicate remaining ARCH effects — GARCH is then applied to the residuals.
- Residual distribution should be approximately normal (Q-Q, histogram).

### 4.2 GARCH Standardized-Residual Diagnostics

*No GARCH results provided. Run `garch_model.garch_diagnostics()` and pass the output to `generate_report()` to embed actual statistics.*

Diagnostics to check:
- Ljung-Box test on standardized residuals: p-values > 0.05 indicate no remaining serial correlation.
- Ljung-Box test on squared standardized residuals: p-values > 0.05 indicate the GARCH model has adequately captured volatility clustering.
